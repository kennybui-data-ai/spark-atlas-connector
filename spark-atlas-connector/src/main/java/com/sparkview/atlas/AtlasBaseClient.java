 ...
 */
package com.sparkview.atlas;

import com.sparkview.atlas.filter.ServicePrincipalAuthFilter;
...
public abstract class AtlasBaseClient {
    ...
    protected AtlasBaseClient(String[] baseUrl, String[] oauthServicePrincipal) {
        if (oauthServicePrincipal != null) {
            if (oauthServicePrincipal.length > 0) {
                this.tenantID = oauthServicePrincipal[0];
            }
            if (oauthServicePrincipal.length > 1) {
                this.clientID = oauthServicePrincipal[1];
            }
            if (oauthServicePrincipal.length > 2) {
                this.clientSecret = oauthServicePrincipal[2];
            }
        }
    ...
    @VisibleForTesting
    protected AtlasBaseClient(Configuration configuration, String[] baseUrl, String[] oauthServicePrincipal) {
        if (oauthServicePrincipal != null) {
            if (oauthServicePrincipal.length > 0) {
                this.tenantID = oauthServicePrincipal[0];
            }
    ...
    void initializeState(Configuration configuration, String[] baseUrls, UserGroupInformation ugi, String doAsUser) {
        ...
        if ((!AuthenticationUtil.isKerberosAuthenticationEnabled()) && tenantID != null && clientID != null && clientSecret != null) {
            final ServicePrincipalAuthFilter authFilter = new ServicePrincipalAuthFilter(tenantID, clientID, clientSecret);
