...
import com.sparkview.spark.atlas.AtlasClientConf
import com.sparkview.spark.atlas.sql.KafkaTopicInformation
import com.sparkview.spark.atlas.utils.{Logging, ReflectionHelper}
  ...
  private def populateValuesFromKafkaStreamWriterFactory(
    ...
    if (isKafkaStreamWriterFactory(writer)) {
      ...
      val params = ReflectionHelper
        ...
        .flatMap {
          // SPARK-25501
          case p: util.Map[String, Object] @ unchecked =>
            Some(p.asScala.map(elem => elem._1 -> elem._2.toString).toMap)
          // Before SPARK-25501
          case p: Map[String, String] @ unchecked => Some(p)
          case _ => None
        }
